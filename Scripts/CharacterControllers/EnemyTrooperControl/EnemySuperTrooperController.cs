...
/// EnemySuperTrooperController:   
///    -To control Enemy Super trooper => the spiky one.
/// Improvements:
///    -Enemy may die with two arrows instead of one.
/// </summary>
public class EnemySuperTrooperController : MonoBehaviour {
	...
	public bool smartZone = false;					// V2.0
	public GameObject sophieReference;				// V2.0
	...
	private bool canSeeSophie = true;
	...
	void Start () {
		...
		sophieReference = GameObject.FindGameObjectWithTag("Player");
	}
	...
	void Update () {
		...
		if (Math.Round(currentTransform.position.x) == Math.Round(targetPointA.position.x)) {
			...
			//
			if (smartZone) {
				float sophieX = sophieReference.transform.position.x;
				Vector3 newWalkTarget = new Vector3(sophieX, walkingTarget.y, walkingTarget.z);
				walk(newWalkTarget);
			}else{
